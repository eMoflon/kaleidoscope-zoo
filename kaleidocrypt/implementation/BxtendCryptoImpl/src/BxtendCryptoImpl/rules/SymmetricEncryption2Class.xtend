...
class SymmetricEncryption2Class extends Elem2Elem{
	...
	def createJavaVariable(String type, String name, int index){
		val param = CryptoJavaFactory.eINSTANCE.createJavaVariableDeclaration
		param.type = type
		param.name = name
		param.index = index
		return param
	...
	def createJavaLiteral(String stringLiteral){
		val literal = CryptoJavaFactory.eINSTANCE.createJavaLiteral
		literal.value = stringLiteral
		return literal
	}
	def createJavaName(String stringName, int index){
		val name = CryptoJavaFactory.eINSTANCE.createJavaName
	...
	def createEncryptWorkflowMethod(JavaCompilationUnit cunit, JavaLiteral nameModePadding){
		...
		val methodInvocation = CryptoJavaFactory.eINSTANCE.createJavaMethodInvocation
		val returnStatement = CryptoJavaFactory.eINSTANCE.createJavaStatement
		...
		val workflowMethod = CryptoJavaFactory.eINSTANCE.createJavaWorkflowMethod => [
			name = "encrypt"
	...
	def createPerformEncryptionOpaqueMethod(JavaCompilationUnit cunit){
		val opaqueMethod = CryptoJavaFactory.eINSTANCE.createJavaOpaqueMethod => [
	...
	override sourceToTarget(String s) {
		...
		.forEach[ c | 
			...
			val targetJavaPackage = corrTargetJcu.getOrCreateTargetElem(CryptoJavaPackage.eINSTANCE.javaPackage) as JavaPackage
			
						
			val corrTarget = c.getOrCreateCorrModelElement(BxtendCryptoImplTransformation.symmetricEncryption2JavaCompilationUnitCorr)
			val cunit = corrTarget.getOrCreateTargetElem(CryptoJavaPackage.eINSTANCE.javaCompilationUnit) as JavaCompilationUnit
			...
			if(!targetModel.contents.contains(targetJavaPackage)){
				...
				val nameModePaddingLiteral = corrNameModePaddingTarget.getOrCreateTargetElem(CryptoJavaPackage.eINSTANCE.javaLiteral) as JavaLiteral

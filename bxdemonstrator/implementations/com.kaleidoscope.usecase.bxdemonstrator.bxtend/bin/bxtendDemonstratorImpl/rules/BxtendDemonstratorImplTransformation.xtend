...
public class BxtendDemonstratorImplTransformation {
	...
	new(URI source, URI target, URI correspondence) {
		...
		if (corrModel.contents.size == 0) {
			corrModel.contents.add(BxtendDemonstratorImplFactory.eINSTANCE.createTransformation)	
		}
	...
	def private void addRules() {
		...
		rules.add(new Grid2Kitchen(sourceModel, targetModel, corrModel))
		rules.add(new Socket2Group(sourceModel, targetModel, corrModel))
		rules.add(new Item2Group(sourceModel, targetModel, corrModel))		
	}
	...
	def deleteUnreferencedSourceElements(){
		...
		detectTargetDeletions().forEach[c |
			...
			// end
			if(!(c.sourceElement instanceof Block))
